...
Run-anywhere portability is a real challenge in bioinformatics, not just for Tapirs. Tapirs has been tested successfully on OSX, and Linux, but problems may still arise. Like most bioinformatics software it is harder but still possible to run on MS Windows machines. We think that a Linux virtual machine or the new Windows Subsystem for Linux are most promising.
